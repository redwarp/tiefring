...
impl TiefringRenderer {
    fn new(
        ...
        let texture_context = Rc::new(TextureContext::new2(device));

        let renderer = Renderer::new2(device, &texture_context, &camera);
        let buffer_cache = BufferCache::new();
        let size = SizeInPx { width, height };
        let white_texture = Rc::new(Texture::new2(
            device,
            queue,
        ...
        let texture_context = TextureContext::new2(device);
    ...
    where
        F: FnOnce(&mut Graphics2),
    {
        self.reset();

        let mut graphics = Graphics2::new(
            self.size,
...
impl<'a> Graphics2<'a> {
        ...
        T: AsRef<str>,
    {
        ...
        let mut operations = self.text_converter.render_operation2(
            text.as_ref(),
            color,
            position,
            &font_for_px,
        ...
        let texture = font_for_px
            .borrow_mut()
            .get_or_create_texture2(&self.device, &self.texture_context);
